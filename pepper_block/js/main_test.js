...
var BlockWorkSpace = function (blockManager, dragScopeName, workspaceName){
    ...
    var timerTes=null;
    var updateTes_ = function(){
        if(self.workAreaElement)
        {
            if(timerTes)
            {
                clearInterval(timerTes);
                timerTes = null;
            }
            var ox = self.offsetX();
            var oy = self.offsetY();
            ...
            var root = $(".box-workspace-transform-root",self.workAreaElement);
            root.css({
                "-webkit-transform":
                  " translate3d("+ox+"px,"+oy+"px,0)",
            });
            timerTes = setInterval(function(){
                root.css({
                    "-webkit-transform":
    ...
    self.offsetX.subscribe(updateTes_);
    self.offsetY.subscribe(updateTes_);
...
ko.bindingHandlers.blockWorkSpaceAutoLayout = {
    ...
    update: function(element, valueAccessor) {
        var blockWs = ko.unwrap(valueAccessor());
        if(blockWs.autoLayoutCb){
            if(blockWs.autoLayoutCbDirty){
                blockWs.autoLayoutCb();
                blockWs.autoLayoutCbDirty = false;
            }
        }
    }
};
    ...
    var setDragGuide = function(block)
    {
        ...
        if(block.valueOut)
        {
            $(".hitAreaValueIn").each(function(k,elm){
                var tgtElm = $(elm).parents(".blockTest");
...
$(function(){
    ...
    function MyModel() {
        ...
        var materialBoxWs = self.blockManager.createBlockWorkSpaceIns("noDroppable","test");

        for(var ii=0; ii < 10; ii++)
        {
            materialBoxWs.addBlock(self.blockManager.createBlockIns("if@basicBlk"));
            materialBoxWs.addBlock(self.blockManager.createBlockIns("if_else@basicBlk"));
            materialBoxWs.addBlock(self.blockManager.createBlockIns("nowTime@basicBlk"));
        }
